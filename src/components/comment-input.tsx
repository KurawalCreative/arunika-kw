...
interface CommentInputProps {
  authorImage?: string;
  authorName?: string;
  value?: string; // controlled input dari parent
  onChange?: (value: string) => void; // update input di parent
  onSubmit: (comment: string) => Promise<void> | void;
  loading?: boolean; // loading dari parent
}
...
export default function CommentInput({
  ...
  const handleSubmit = async () => {
    ...
    if (loadingProp !== undefined) {
      await onSubmit(text);
      onChange?.(""); // reset input di parent
    } else {
      setLoading(true);
      try {
        await onSubmit(text);
  ...
  return (
    <div className="border-gray-200 dark:border-slate-700 pt-4">
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="bg-orange-500 flex items-center rounded-xl px-4 py-2 font-semibold text-white hover:bg-orange-600 disabled:opacity-50"
        >
